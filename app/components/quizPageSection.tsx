...
import LeaderBoard from "./leaderBoard"; // Optional, add only if used
...
function getBadge(score: number, totalQuestions: number): string {
  ...
  if (percentage >= 70) return "🥈 Silver Badge";
  if (percentage >= 50) return "🥉 Bronze Badge";
  return "😅 Try Again Badge";
}
...
export default function QuizPageSection({ Quizes, levelNumber, levelTitle, player }: any) {
  ...
  useEffect(() => {
    if (timeLeft === 300) {
      toast("⚠️ You only have 5 minutes left. The quiz will auto-submit after 5 minutes.", {
        duration: 6000,
        position: "top-center",
      });
    }
  ...
  const handleNextLevel = async () => {
    if (!player?.Playerpoint && player?.Playerpoint !== 0) {
      ...
      {showIntroModal && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-lg text-center">
            ...
            <p className="mb-6">
              This Beginning Level quiz consists of {len} questions and lasts 10 minutes.
              Spend about 1 minute per question. The quiz will auto-submit when time is up.
            </p>
            <button
        ...
        {questionNumber < len ? (
          <div className="md:py-16 pt-8 pb-28">
            <div className="container flex justify-between flex-wrap">
              ...
              <p className="mb-6">Question : {questionNumber + 1}/{len}</p>
            </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <div className="flex flex-wrap-reverse justify-center gap-8 items-center">
                  <div className="flex flex-col gap-8 mt-6 justify-center">
                    ...
                    <div className="bg-green-50 rounded border-2 border-green-300 flex flex-col gap-4 items-center px-6 py-4">
                      <p className="mt-4 text-xl">🎖 Badge Earned</p>
                      <h1 className="text-3xl font-semibold">{getBadge(score, len)}</h1>
                    </div>
                ...
                {/* Optional: Add leaderboard below if needed */}
                {/* <LeaderBoard /> */}
              </div>
            </div>
          </div>
      ...
      {showTimeUpModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-8 text-center shadow-xl max-w-md w-full">
            ...
            <p className="text-gray-700 mb-6">Your quiz time has ended. Your score has been saved.</p>
